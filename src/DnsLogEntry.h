...
class DnsLogEntry : public iCache {
    private:
        std::string Fqdn;
        std::map<boost::asio::ip::address, uint32_t> Ips;
        bool Debug;
    ...
    public:
        ...
        DnsLogEntry(const std::string inFqdn, const std::map<boost::asio::ip::address, uint32_t> inIps, const bool inDebug = false):
